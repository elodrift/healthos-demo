/**
 * Local intent matcher. Runs entirely in the browser, no network, no model.
 *
 * Why this exists at all: the demo gets handed to a stranger who types whatever
 * they want. Anything that touches a medical rule, a target number, or the
 * event log has to be answered deterministically — a probabilistic model must
 * never be the thing that decides whether grapefruit is safe. So the matcher
 * owns every intent with consequences, and only genuinely open-ended chat is
 * allowed to fall through to the model.
 */
